...
- **Step 3 — Propose the concrete bootstrap as `✏️ [change]`.** Single proposal listing the canonical substitution map (all 10 B-024 placeholders with the user's values) + every file created/modified (sourced from `templates/manifest.yaml` entries where `exported_by_starter: true`) + `git init -b main` + `uv lock` + first commit. After `gogogo!`, reuse `scripts/render-example.sh` substitution logic for the `mv` + `sed` passes, then run `uv lock` so the committed lockfile satisfies CI's `uv sync --frozen` (BOOTSTRAP.md invariant), and `git init -b main` so the initial branch matches the later push/branch-protection regardless of the host's `init.defaultBranch`. If Q5 = create-now: detect `gh auth status` and surface `gh repo create` + branch protection + push as a SEPARATE follow-up `✏️ [change]` (split keeps interactivity awkwardness contained).
...
### D-025 (2026-05-20) Kit repo made public; onboarding validated end-to-end; site on cPanel

**Chose:** Make `denisbalon/phoenixtemplate` a **public** repo, and record the real-world deployment outcome of the B-039/D-021 onboarding mechanism after its first live test.
**Considered:** keep the kit private and host `ONBOARDING_PROMPT.md` only on the website (rejected — the canonical bootstrap prompt + the website's "For Claude" block both point at the `raw.githubusercontent.com/.../main/ONBOARDING_PROMPT.md` URL, which 404s for anonymous fetchers when the repo is private; the whole mechanism assumes a public raw URL); keep the GitHub Pages deployment D-021's prose anticipated (superseded — the site shipped on cPanel instead).
**Why:** The first live test of `Start a project with phoenixtemplate.com` (in an empty dir) surfaced two infrastructure prerequisites that the B-039 design implicitly assumed but didn't state: (1) **the kit repo must be public** — a private repo makes the raw `ONBOARDING_PROMPT.md` URL return 404 anonymously (and the GitHub API hides the repo's existence), breaking the second fetch in the chain; (2) **the apex domain must not carry a dead `AAAA` record** — an IPv6 record pointing at an address that refuses connections made `WebFetch` (IPv6-preferring) fail with `ECONNREFUSED` even though IPv4 served fine. Both were fixed (repo flipped public; the apex AAAA removed at the host), after which the full chain — fetch site → follow to the raw prompt → run Steps 1–4 — worked. The kit being public is the intended state for an adoption-oriented kit; private was an oversight. The website itself lives in the separate `denisbalon/phoenixtemplate.com` repo and is deployed to cPanel over FTPS (see that repo's D-001), superseding the GitHub-Pages assumption in D-021's "Implemented in" note.
**Implemented in:** v1.40.0. Touches: `docs/spec.md` (this D-025 entry; B-039 Test field updated to mark the live-flow validated and name the two prerequisites); housekeeping in "Open project-level decisions" (resolved items moved out). Repo visibility change (`gh repo edit --visibility public`) and the DNS AAAA removal are infrastructure actions, not file changes. No B-block — decision/record only.

## Open project-level decisions

Resolve as they come up. Move resolved entries to the Decision log above.

- [x] **De-personalize the template.** Original residue was `bootstrap.sh` menu header hardcoded to `phoenixtgstat_bot` + baked-in Telegram/Meta/Keitaro validators. (PROJECT_STARTER.md item A1–A3.) Status — fully resolved:
...
- [x] ~~**Split PROJECT_STARTER.md** into focused docs (`BOOTSTRAP.md`, `WORKFLOW.md`, `HARNESS_QUIRKS.md`, …) with PROJECT_STARTER.md as the index. (Item F22)~~ — **Resolved in v1.22.0–v1.26.0 (B-025).** Split into `TEMPLATE_INVENTORY.md` / `DEPLOY_BASELINE.md` / `HARNESS_QUIRKS.md` / `WORKFLOW.md` / `BOOTSTRAP.md`; PROJECT_STARTER.md is now a thin index.
- [x] ~~**Decide PROJECT_STARTER.md role in cloned projects.** Snapshot-at-bootstrap vs. thin pointer. (Item F23)~~ — **Resolved in v1.27.1 (D-012).** Settled as a permanent thin-index entry-point (owns the Template version line + changelog + docs table).
