...
**Rule:** `scripts/check-rule-consistency.sh` verifies that named "rule regions" appear byte-for-byte identically across the three canonical doc tiers named by B-021 (`PROJECT_STARTER.md` §2, `templates/CONTRIBUTING.md`, `templates/CLAUDE.md`). Each region is bracketed in each file by HTML-comment anchors `<!-- C4:<region>:start -->` and `<!-- C4:<region>:end -->`. The script extracts content between matching anchors in each file, diffs every pair against the first file as reference, and exits non-zero on any drift or missing region (printing a unified diff identifying the drift). The three regions in v1.19.0 are: (i) `gate-clause` — the one-sentence "Do NOT take any state-mutating action unless the user's CURRENT message contains the literal substring `gogogo!`" rule; (ii) `verb-table` — the markdown table mapping `<verb> gogogo!` phrases to actions; (iii) `bare-gogogo` — the bare-`gogogo!` clarification prompt. The script is wired into `.github/workflows/template-self-test.yml` as a job step that runs before the smoke test on every push and PR to `main`. Adding a region in a future version: add the anchored region (verbatim text) to all three files in the same commit and append the region name to the script's `REGIONS` array.
...
**Rule:** As of v1.22.0, the meta-repo root ships `PROJECT_STARTER.md` plus three companion docs that own subsets of what was previously a single 1100-line file: `TEMPLATE_INVENTORY.md` (file/folder layout + `templates/` reference — was §3 + §4 of PROJECT_STARTER.md), `DEPLOY_BASELINE.md` (VPS deploy + CI/CD baseline + credential handling — was §6 + §7 + §13), `HARNESS_QUIRKS.md` (Claude Code harness gotchas + `bootstrap.sh` design — was §12 + §14). PROJECT_STARTER.md retains the bootstrap checklist (§1), the `gogogo!` gate + 5-step workflow (§2), decision bank (§5), conventions (§9), memory seed (§10), PR review heuristics (§11), and audit trail / Decision log / Template changelog (§8 + tail). The split sections in PROJECT_STARTER.md retain their heading + a one-line pointer (`**Moved to [X.md](X.md) in v1.22.0.**`) so readers scrolling through find the new location and section numbers stay stable. All four root docs ship in the export archive (`scripts/export-starter.sh` `ROOT_DOCS` array) so the cross-links resolve in the extracted layout; `VIRTUAL_TEMPLATES_FILES` in `scripts/check-doc-references.sh` (B-023) is the parallel allowlist used by the doc-reference linter to recognize the same export model. Further splits planned: `WORKFLOW.md` extracts §2 + §9 + §10 + §11 in v1.22.1 (coordinated with B-022 C4-linter target relocation, since the rule regions live in §2); `BOOTSTRAP.md` extracts §0 + §1 + §5 in v1.22.2, at which point PROJECT_STARTER.md becomes a thin ~50-line index.
...
- **Stage in three commits, not one.** A single commit splitting 1100 lines five ways is hard to review and easy to silently drop content in. The trio of linters (B-022 / B-023 / B-024) shipped in v1.19.0–v1.21.0 was the prerequisite — they mechanically catch the kinds of drift a careless surgical move would otherwise introduce. v1.22.0 handles the three "safe" extractions that don't touch the C4-linter anchored regions (those are in §2). v1.22.1 handles WORKFLOW.md and the coordinated B-022 linter retarget. v1.22.2 finishes with BOOTSTRAP.md and the thin-index reduction.
...
**Test:** automated — (a) the doc-reference linter (B-023) passes, proving all PROJECT_STARTER.md → companion-doc links resolve in the meta-repo; (b) `./scripts/export-starter.sh` produces an archive containing all four files at the staged root level (verifiable via `tar -tzf <archive> | grep -E 'project-starter-v[^/]+/(PROJECT_STARTER|TEMPLATE_INVENTORY|DEPLOY_BASELINE|HARNESS_QUIRKS)\.md$'` showing four matches); (c) the smoke test (B-014) passes on the archive (template instantiation still works end-to-end); (d) `wc -l PROJECT_STARTER.md` is ~800 (down from 1121 pre-split).

**Status:** frozen (v1.22.0 — three of five planned files shipped). Will be updated in v1.22.1 and v1.22.2 as the remaining two files extract.
