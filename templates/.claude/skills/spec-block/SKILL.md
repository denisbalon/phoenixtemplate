...
5. **Stop.** Do not commit. Do not bump VERSION. The Block insert is sub-stage (a) of the next atomic commit on the on-branch per-node `gogogo!` cadence; the user will trigger that commit (its own `gogogo!`) when ready.
