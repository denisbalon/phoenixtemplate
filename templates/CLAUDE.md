...
When the proposed action is a fresh-work feature change, the on-branch 6-step atomic sequence (one `gogogo!`) is: **branch** (`<type>/<slug>-v<X.Y.Z>` off `main`) → **spec** (incl. decision log if architectural) → **bump versions + CHANGELOG entry** → **code** → **commit + push to the feature branch** → **open the PR ready** (no `--draft`). Claude stops at step 6. Subject ends `v<X.Y.Z>`. Every change bumps `VERSION` AND adds a `CHANGELOG.md` entry. Every commit is pushed to origin (to the feature branch — never to `main`; the local `.githooks/pre-push` hook blocks it) in the same turn — no local-only commits. Address-review iterations on an existing branch skip steps 1 + 6 (branch + PR already exist) and keep steps 2–5. Merge is a separate `gogogo!`, atomic over three sub-steps (`gh pr merge --rebase --delete-branch` → `git checkout main && git pull --ff-only origin main` → deploy); the deploy step must NOT be surfaced as a separate `gogogo!` after the merge.
