...
**Review is out-of-band.** Review happens in a separate session with whatever reviewer the user prefers, pointed at `docs/pr_review_instructions.md` and the open PR. Claude does not dispatch reviewers from the authoring session; in a dedicated review session, if Claude is the reviewer, it prepares comment-ready GitHub review text and offers to post it as a separate gated action. The literal command `review-post!` (B-045) is the one exception — it authorizes reviewing every PR open at that moment and publishing each package immediately, scoped strictly to per-commit comments, inline review comments, and the one `event=COMMENT` overall review; it authorizes no other GitHub or local mutation.
...
When the proposed action is on-branch feature work, the cadence is **per-node `gogogo!` with N≥1 commits per branch**: **stake the feature branch** (one `gogogo!`; `<type>/<slug>` off `main` — no version suffix on the branch name) → **N atomic commits on the branch** (each its own `gogogo!`; per-commit shape: spec? (incl. decision log if architectural) → bump versions (VERSION + language-specific markers move together) → own `## v<X.Y.Z> — YYYY-MM-DD` CHANGELOG entry → code → commit + push to feature branch) → **open the PR** (one `gogogo!`; title `<type>: <bundle description> v<X.Y.A>..v<X.Y.B>`, collapses to `v<X.Y.A>` for single-commit branches; no `--draft`). Commit subject ends `v<X.Y.Z>`. Every commit is pushed to origin (to the feature branch — never to `main`; the local `.githooks/pre-push` hook blocks it) in the same turn — no local-only commits. **No WIP commits** — every commit deliberate. Address-review iterations on an existing branch are more atomic commits, each its own `gogogo!`, with the same per-commit shape. Merge is a separate `gogogo!`, atomic over three sub-steps (`gh pr merge --rebase --delete-branch` → `git checkout main && git pull --ff-only origin main` → deploy); the deploy step must NOT be surfaced as a separate `gogogo!` after the merge.
