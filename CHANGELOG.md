...
**Address Codex review of PR #24 (3 findings on the audit skill) — B-052 / D-035.** **Block:** Phase 5 said stage *only* `docs/audit-<id>.md` while its subject carried `v<X.Y.Z>` “following P3”, and `templates/CONTRIBUTING.md` requires every commit to bump the version markers + `CHANGELOG.md` — the two could not both hold. Reconciled toward the kit shape: the audit commit now stages the artifact **plus** `{{VERSION_FILES}}` + `CHANGELOG.md`, the only-file check expects exactly that set, and B-052's Test mirrors it; it is the ordinary per-commit shape, not an exception. **Strong:** the post-commit “record the artifact commit SHA in the header” step wrote into the artifact *after* committing it, leaving a dirty tree or a stale placeholder — removed, along with the `artifact commit:` skeleton line; the git commit is the record and the header already carries the audited `BASE_SHA`. **Strong:** slice D swept `docs/runbooks/**` but the kit ships `templates/docs/runbook.md` → `docs/runbook.md` in consumers, so the default runbook fell outside coverage — slice D now includes `docs/runbook.md` and discovers further runbooks from project docs. Global `~/.claude/skills/audit/` copy re-synced. All 6 linters + smoke-test green. Patch bump.
...
The package shape is unchanged from B-043: per-commit coverage across `main..HEAD` including an explicit "no findings on `<sha>` — `<subject>`" comment on every clean commit, inline review comments where a finding attaches to specific lines, exactly one overall PR review with `event=COMMENT`, and a separate read-back query confirming every write actually landed on GitHub.

**Hard scope boundary.** `review-post!` authorizes three artifact classes and nothing else — per-commit comments, inline review comments, and the one `event=COMMENT` overall review. PR creation, merge, close, approval, labels, PR-description edits, issue actions, local repository changes, and every other GitHub mutation keep their existing `gogogo!` gate. A plain request to review, draft, or prepare comments carries no publishing authorization at all: it produces the package and stops before any GitHub write, exactly as B-043 specifies.

**Why an exception at all.** The gate exists to keep a misread cheap. For merges, force-pushes, deletes, and deploys a misread is expensive and often irreversible, so those stay gated per-action. Review comments are the opposite: additive, non-destructive, editable and deletable after the fact, and — under B-043 — already fully inspected in-session before any token gets typed. Charging both classes the same confirmation price is what made bulk review passes annoying enough that reviews stopped landing on GitHub at all (recorded as DEV_NOTES item 7), which is worse for the audit trail than the fan-out risk. The exception is therefore drawn along blast radius rather than along state-mutation.

**What B-040 keeps.** `review-post!` is a *distinct literal token*, not a widening of `gogogo!`. Authorization is still carried by an exact string the user types — never by natural-language imperative, never by Claude's confidence that it understood a request. B-040's anti-misread core is untouched; only its "the **only** authorization channel" clause is amended to "only, with exactly one narrow scoped exception."

**Accepted cost, chosen explicitly.** One token publishes to an unbounded open-PR set with no per-PR checkpoint — `review-post!` typed while five stale PRs are open publishes five reviews. A narrower variant was surfaced with this tradeoff spelled out (bare `review-post!` → current branch's PR, `review-post! <PR#>` → explicit, `review-post! --all` → fan-out) and rejected in favor of the sweep, because the multi-PR pass is the ergonomic point of the command. See D-029.

Touches: `docs/spec.md` (B-045 added frozen; D-029 added; B-043 Status gains a refined-by line; B-040 Status gains an amended-by line); the C4 `gate-clause` + `proposal-format` regions byte-exact across `WORKFLOW.md` + `templates/CONTRIBUTING.md` + `templates/CLAUDE.md` (B-022 verified green); `templates/docs/pr_review_instructions.md` (preamble ¶ + output-contract item 1, using the wording from the source spec); `PROJECT_STARTER.md` (Template-version header + changelog row per B-002); `CHANGELOG.md` + `VERSION`. Commit #2 (v1.44.1, its own `gogogo!`) sweeps the non-C4 prose mirrors at `WORKFLOW.md` §"PR review", `templates/CONTRIBUTING.md` §6 + §review-heuristics, and the `templates/CLAUDE.md` review one-liner. All 3 linters green (C4 / spec / doc-refs). Minor bump per WORKFLOW.md (new frozen spec block + C4 region edit).

## v1.43.5 — 2026-05-28

**Close rule gap surfaced by PR #9 re-review — PR title + body update on each address-review commit (B-044 / D-028 address-review iteration #2).** The v1.43.4 commit fixed the two Strong findings from the original review, but the v1.43.0 rule as shipped only described title-range generation at PR-open time; it didn't explicitly say "and gets updated when address-review commits extend the range." On PR #9 itself the reviewer caught this — after `eee3756` landed, the PR title still said `v1.43.0..v1.43.3` and the body said "4 atomic commits", contradicting the very rule the PR introduces. The honest reading is that the rule had an implicit clause that the operator (me) failed to apply because it wasn't written down explicitly. This commit writes it down: the C4 `proposal-format` region across `WORKFLOW.md` + `templates/CONTRIBUTING.md` + `templates/CLAUDE.md` now appends an explicit obligation to the title-range claim — **"each address-review commit that extends the range obligates a `gh pr edit <PR#> --title \"...\" --body \"...\"` in the same turn as the commit's push, so PR metadata stays in sync with the branch's commits"** (B-022 verified green byte-exact). The WORKFLOW.md §"The on-branch workflow" node (4) Address-review iterations + the templates/CONTRIBUTING.md mirror + the docs/spec.md B-044 Rule clause (4) all get the same addition for consistency. The metadata edit on PR #9 itself happens as a separate `gh pr edit` action authorized by the same `gogogo!` that authorized this rule amendment (the user's multi-action authorization for option 2 of the proposal). PROJECT_STARTER.md Template-version header bumped to v1.43.5 + changelog row added (per B-002). All 3 linters green (C4 / spec / doc-refs). Patch bump (rule-text clarification; no behavior change to B-044's frozen rule — closing an implicit-vs-explicit gap, not adding new mechanics).

## v1.43.4 — 2026-05-28

**Address PR #9 review findings — C4 commit-shape ordering bug + B-005 Rule stale-numbered-step (B-044 / D-028 address-review iteration).** Both Strong findings on the on-branch per-node cadence rollout PR. **Finding #1 (Strong, C4-loaded surface — high impact):** the `proposal-format` C4 region across `WORKFLOW.md` + `templates/CONTRIBUTING.md` + `templates/CLAUDE.md` carried `spec? → bump → code → CHANGELOG section → commit`, but the non-C4 expansion at `WORKFLOW.md:113-119` (and B-044 spec text, and `templates/CONTRIBUTING.md` §"On-branch per-node cadence") puts CHANGELOG BEFORE code: `(a) spec → (b) bump → (c) CHANGELOG → (d) code → (e) commit`. The (a)-(e) order is the original spec; the C4 wording was the bug, inherited verbatim from phoenixcnc PR #13's C4 text (their PR has the same bug — to be reported back upstream). Fixed by surgical swap "code → own `## v<X.Y.Z>` CHANGELOG section" → "own `## v<X.Y.Z>` CHANGELOG section → code" in all 3 byte-exact-synced files (B-022 verified green). **Finding #2 (Strong):** `docs/spec.md:49` (B-005 Rule) said "Step 5 of the `gogogo!` workflow (deploy) is a no-op for `phoenixtemplate`" — but after B-044, deploy is no longer "Step 5"; it's the third sub-step of the merge `gogogo!` (which is node 5 of the per-node cadence). Rewrote the Rule to describe deploy as "the deploy sub-step of the merge `gogogo!` (third of three sub-steps per B-044, atomic over `gh pr merge --rebase --delete-branch` → `git checkout main && git pull --ff-only origin main` → deploy)" while preserving the frozen behavior (deploy still no-op for this meta-repo; documented-as-no-op rather than skipped so the merge `gogogo!` stays atomic over the same three sub-steps as consumer projects). Rationale + Test fields left intact (they didn't reference numbered steps). PROJECT_STARTER.md Template-version header bumped to v1.43.4 + changelog row added (per B-002). All 3 linters green (C4 / spec / doc-refs). Patch bump (address-review fixes; rule wording fixes, no behavior change to either frozen rule).

## v1.43.3 — 2026-05-28

**Sweep stale 6-step refs in active workflow surfaces (commit #4 of the per-node-cadence rollout — B-044 / D-028; final commit on the rollout branch before PR-open).** Three active surfaces still pointed at the bundled-6-step model and would have misdirected the user at exactly the wrong moments. Touches: `.githooks/pre-push` line 18 — the post-block guidance message printed when someone tries to push directly to `main`; stale text here is high-impact because it's seen at the exact violation moment. Updated to "See CLAUDE.md / WORKFLOW.md for the on-branch per-node gogogo! cadence." `templates/.githooks/pre-push` line 18 — same message in the hook shipped to consumer projects via `make install-hooks`. `templates/.claude/skills/spec-block/SKILL.md` line 39 — closing "Stop" step of the `/spec-block` skill, loaded into Claude's context every time the skill runs; rewrote so the Block insert maps to **sub-stage (a) of the next atomic commit** on the on-branch per-node `gogogo!` cadence (was "step 2 of the next `gogogo!` on-branch 6-step workflow"). After this commit, `git grep -nE "6-step|on-branch 6-step"` returns nothing in tracked files outside historical CHANGELOG entries (`CHANGELOG.md` + `templates/CHANGELOG.md` — none after this rollout), `PROJECT_STARTER.md` changelog table rows for v1.41.0 / v1.42.0 (audit trail), and the superseded `B-042` / `D-026` spec blocks in `docs/spec.md` (audit trail). PROJECT_STARTER.md Template-version header bumped to v1.43.3 + changelog row added (per B-002). Branch is now ready for PR-open as a separate `gogogo!` (title `feat: per-node gogogo! cadence v1.43.0..v1.43.3`). All 3 linters green (C4 / spec / doc-refs). Patch bump (active-surface sweep; no spec/behavior change beyond what landed in v1.43.0).

## v1.43.2 — 2026-05-28

**Sweep stale 6-step refs in bootstrap/template surfaces (commit #3 of the per-node-cadence rollout — B-044 / D-028).** v1.43.0 + v1.43.1 covered the C4 trio + WORKFLOW.md + templates/CONTRIBUTING.md + root CONTRIBUTING.md + docs/spec.md, but bootstrap-tier and template-shipped surfaces still described the bundled 6-step (or older 5-step) flow. These files are linked from `README` / `PROJECT_STARTER` as live user-facing process docs (and `templates/CLAUDE.md` is loaded every consumer session), so anyone bootstrapping the kit, importing the process layer, or just running a Claude session in a consumer project would have received contradictory guidance after v1.43.0 lands without this sweep. Touches: `PROJECT_STARTER.md` lines 6 (kit-index process-layer description) + 13 (WORKFLOW.md doc-table row) + Template-version header (1.42.1 → 1.43.2) + Template changelog backfill rows for v1.43.0 / v1.43.1 / v1.43.2 (process miss from commits #1/#2 fixed retroactively per B-002 — those commits should have bumped PROJECT_STARTER.md too). `BOOTSTRAP.md` lines 11 (Process-layer scope table), 19 (reading-order item 2), 155 (first-commit template body — the wording NEW projects bootstrapping from the kit see in their seed commit). `MIGRATION.md` line 32 ("Importing just the process layer"). `README.md` line 13 (top-level Process row). `templates/CLAUDE.md` line 43 — full rewrite of the non-C4 gate-section paragraph for per-node cadence (the four C4 anchor regions are untouched; only surrounding prose changed; C4 linter still passes byte-exact). `templates/docs/karpathy-claude-rules.md` lines 72-76 — rules-mapping rewritten from old "Step 2 / Step 4 / Step 5" numbering to per-commit atomic sub-stages (a)/(d)/(e) as defined in the new WORKFLOW.md node (2). `templates/docs/spec.md` line 11 — Process & versioning paragraph rewritten (the template that ships to consumers; mirrors the docs/spec.md fix in v1.43.1). `templates/CHANGELOG.md` seed wording — "part of step 3 of the 6-step workflow" → "part of sub-stage (c) of each atomic commit on the on-branch per-node cadence". Out of scope (deferred to v1.43.3 per the rollout plan): active workflow surfaces — `.githooks/pre-push` + `templates/.githooks/pre-push` (the message printed when blocking direct-to-main push — wrong moment to misdirect) + `templates/.claude/skills/spec-block/SKILL.md` (loaded into Claude's context every time `/spec-block` runs). After v1.43.3, `git grep -nE "6-step|on-branch 6-step"` will return nothing in tracked files outside historical CHANGELOG entries and the superseded B-042 / D-026 spec blocks (audit-trail). All 3 linters green (C4 / spec / doc-refs). Patch bump (docs sweep; no spec/behavior change beyond what landed in v1.43.0).

## v1.43.1 — 2026-05-28

**Sweep stale 6-step refs in meta/process tier (commit #2 of the per-node-cadence rollout — B-044 / D-028).** v1.43.0 updated the C4 `proposal-format` region + WORKFLOW.md prose for §"The on-branch workflow (per-node cadence)" + §Branching + §Pull requests, but binding workflow text in the non-C4 sections of `templates/CONTRIBUTING.md` + root `CONTRIBUTING.md` + remaining `WORKFLOW.md` prose + `docs/spec.md` Process & versioning paragraph still described the bundled 6-step (or even older 5-step) flow. Without this sweep, readers and agents would see contradictory binding instructions after v1.43.0 lands, and the new rule wouldn't actually be coherent. Touches: `templates/CONTRIBUTING.md` — Canonical-scope paragraph (line 5), Quick rules cheat-sheet rows (lines 11-15), Principles bullets (lines 22-25 + 2 new bullets: branch-name-no-suffix and no-WIP-commits), Phase frequency table (lines 35-39), TL;DR (lines 45-50), §1 "Set up the feature branch" prose (line 65), §3 "Open the pull request" intro (line 111), and full rewrite of §"Mandatory on-branch 6-step sequence" → §"On-branch per-node `gogogo!` cadence" (line 269+) mirroring WORKFLOW.md's new 5-node life-cycle with per-commit shape (a)-(e) inside node (2). Root `CONTRIBUTING.md` (line 7) — meta-repo workflow pointer updated. `WORKFLOW.md` — line 3 canonical-source description, line 5 B-021 scope clause, Phase frequency table (lines 314-318), and `gogogo_gate_workflow.md` auto-memory-seed row (line 391) updated. `docs/spec.md` Process & versioning paragraph (line 15) rewritten to describe per-node cadence + corrects the inaccurate "VERSION is single source of truth, never duplicated in prose" wording (since B-042 the three markers move together for Python projects: `VERSION` + `pyproject.toml` + `__init__.py`; the meta-repo carries only `VERSION` because the other markers are template content). Out of scope (deferred to v1.43.2 / v1.43.3 per the rollout plan, each its own `gogogo!`): bootstrap/template surfaces (PROJECT_STARTER / BOOTSTRAP / MIGRATION / README / karpathy / templates/docs / templates/CLAUDE.md non-C4 paragraph / templates/CHANGELOG.md seed) and active surfaces (.githooks/pre-push + templates/.githooks/pre-push + spec-block SKILL.md). All 3 linters green (C4 / spec / doc-refs).
