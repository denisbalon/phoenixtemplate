...
|---|---|---|
| **Process** (stack-agnostic) | Any project | `gogogo!` passphrase gate with propose-and-confirm semantics (Claude proposes concretely, user `gogogo!`s the proposal), on-branch per-node `gogogo!` cadence (N≥1 atomic commits per branch; each of stake-branch, each commit, open-PR, each address-review iteration, and merge is its own `gogogo!`; per-commit shape spec? → bump → CHANGELOG → code → commit + push) + atomic merge `gogogo!` (rebase-merge → pull → deploy), spec-block format (`B-NNN` blocks + Decision log), Karpathy's four LLM-coding pitfalls as standing rules, reviewer-agnostic PR review rubric, local `.githooks/pre-push` block on direct pushes to `main`. |
